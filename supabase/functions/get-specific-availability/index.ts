    ...
    async start(controller) {
      ...
      try {
        ...
        if (!business_id || (!date && !time)) {
          sendEvent({ error: 'business_id and at least date or time are required' });
          controller.close();
          return;
        }
        ...
        if (businessError || !businessSettings) {
          sendEvent({ error: 'Business not found' });
          controller.close();
          return;
        }
        ...
        // Send success event immediately after API call
        sendEvent({ status: 'success', message: 'Calendar data fetched successfully' });
