-- Drop ALL existing RLS policies on call_logs (including any from previous failed migrations)
DO $$ 
DECLARE 
  pol RECORD;
BEGIN
  FOR pol IN 
    SELECT policyname 
    FROM pg_policies 
    WHERE tablename = 'call_logs' AND schemaname = 'public'
  LOOP
...
-- Create a separate secure function for admin access with mandatory audit logging
...
BEGIN
  ...
  -- Log the admin access with justification
  ...
  -- Return masked/limited data for admin view
  RETURN QUERY
  SELECT 
    cl.id,
    cl.business_id,
...
-- Add audit trigger for UPDATE and DELETE operations only
DROP TRIGGER IF EXISTS audit_call_logs_modifications ON public.call_logs;
CREATE TRIGGER audit_call_logs_modifications
